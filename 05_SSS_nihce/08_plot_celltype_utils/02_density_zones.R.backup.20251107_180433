...
calculate_density_zones <- function(
  ...
  col_range_expanded <- c(
    col_range[1] - col_margin, 
    col_range[2] + col_margin
  )
  row_range_expanded <- c(
    row_range[1] - row_margin, 
    row_range[2] + row_margin
  )
  
  cat(sprintf("   ✅ 密度计算范围:\n"))
  cat(sprintf(
    "      原始: col [%.1f, %.1f], row [%.1f, %.1f]\n",
    col_range[1], col_range[2], 
    row_range[1], row_range[2]
  ))
  cat(sprintf(
    paste0("      扩展: col [%.1f, %.1f], row [%.1f, %.1f] ",
           "(边距=%.0f%%)\n"),
  ...
  spot_zones <- df %>%
    ...
    dplyr::mutate(
      col_idx = sapply(
        col, 
        function(x) which.min(abs(kde_result$x - x))
      ),
      row_idx = sapply(
        row, 
        function(y) which.min(abs(kde_result$y - y))
      )
    ) %>%
    dplyr::rowwise() %>%
    dplyr::mutate(
    ...
    dplyr::left_join(
      density_df %>% 
        dplyr::select(col, row, density_zone, density_norm),
  ...
  if (any(is.na(spot_zones$density_zone))) {
    ...
    for (idx in na_spots) {
      ...
      distances <- sqrt(
        (density_df$col - spot_col)^2 + 
        (density_df$row - spot_row)^2
      )
      valid_idx <- which(!is.na(density_df$density_zone))
      
      if (length(valid_idx) > 0) {
        nearest_valid <- valid_idx[which.min(distances[valid_idx])]
        spot_zones$density_zone[idx] <- 
          density_df$density_zone[nearest_valid]
        spot_zones$density_value[idx] <- 
          density_df$density_norm[nearest_valid]
      }
    }
  }
